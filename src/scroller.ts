...
function setScrollParams(
  animatorInfos: ScrollerAnimator[],
  position: Position
) {
...
function getTopmostScrollAnimator(
  animatorInfos: ScrollerAnimator[],
  position: Position
): ScrollerAnimator | null {
  let current = document.elementFromPoint(position.x, position.y);

  while (current) {
    const scrollAnimator = animatorInfos.find(
      (p) => p.scrollerElement === current
    );
    if (scrollAnimator) {
...
export default (containers: IContainer[], maxScrollSpeed = maxSpeed) => {
  const animatorInfos = containers.reduce(
    (acc: ScrollerAnimator[], container: IContainer) => {
  ...
  return ({
    ...
    if (draggableInfo) {
      ...
      if (overlappingAnimators.length && overlappingAnimators.length > 1) {
        ...
        const topScrollerAnimator = getTopmostScrollAnimator(
          overlappingAnimators,
